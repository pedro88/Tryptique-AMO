...
const addListener = () => {
	document.querySelectorAll(".logo").forEach((button) => {
        ...
        container.innerHTML = `<div>${message}</div>`;
			...
			const etToi = () => {
				// Vérifier ou créer le conteneur
				let container = document.getElementById("outro-message");
				if (!container) {
					container = document.createElement("div");
					container.id = "outro-message";
				...
				// Création et ajout du bouton "Et Toi?"
				console.log("et toi fct");
				const existingEtToiButton =
					document.getElementById("etToi-btn");
				if (existingEtToiButton) existingEtToiButton.remove();

				const etToiButton = document.createElement("button");
				etToiButton.textContent = "Et toi?";
				etToiButton.id = "etToi-btn";
				container.appendChild(etToiButton);

				// Gestion de l'événement "click" sur le bouton "Et Toi"
				etToiButton.addEventListener("click", () => {
					socket.emit("etToiChoice", logoId);
					etToiButton.remove();
					container.remove();
					outro();
					addListener();
